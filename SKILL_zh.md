...
**信息完整度模拟规则**（③⑤⑥档通用）：
拿到需求后，不写完整提示词，只做快速扫描，输出「信息完整度报告」：

```
✅ 已明确：日期格式、农历、宜忌、时辰吉凶、磨砂半透明、开机自启
⚠️ 不确定：①缩放方式（自由拖拽/预设尺寸？）②天气数据源 ③时辰吉凶突出方式（高亮/颜色？）
❌ 缺失：数字时钟位置、字体偏好、节假日数据来源
```

用户看到缺口后，指哪补哪。说"不用"则继续当前流程。不开新对话，不重跑，就在当前闸门补。

**为什么不用全量草稿**：太重。信息完整度报告 100-200 字，用户三秒扫完，比空对空问"要不要深化"具体得多。
