---
name: "super-prompt"
description: "提示词工程工具：反歧义拦截×联网预搜索×分层引导×ELO排名，从一句话到生产级提示词。适用于Claude/GPT/Agent系统提示词、SOP、Agent指令。支持输出流程图、架构图、思维导图、播客、文档等多形式交付。触发词：写提示词、精铸提示词、优化prompt、写系统提示词。"
license: MIT
---
...
| 档位 | 用户输入特征 | 走什么流程 |
|------|------------|-----------|
| **①纯一句** | "帮我写个提示词"，什么都没说 | 四层漏斗全走：收问题→拆假设→挖本体→找差集 |
| **②缺参数** | 有方向但缺关键信息（如"做个记账App"没说平台） | 收问题+拆假设精简，重点挖本体补关键信息 |
| **③需求清晰** | 描述完整、逻辑自洽，不追问也能理解（如黄历280字） | 确认理解后，先模拟信息完整度（见下方规则），然后问"要补哪些缺口？"→用户指哪补哪；说"不用"则继续闸门零 |
| **④诊断型** | "帮我看看这个提示词有没有问题" | 直接反歧义18条扫描，不走过场不走四层漏斗 |
| **⑤已有提示词优化** | 拿了现成提示词来优化 | 审查模式：全量预搜→批量抛优化点→用户选。同样先模拟信息完整度→问"要补哪些缺口？" |
| **⑥完整文档** | 完整PRD/方案/结构化文档 | 审查模式全量预搜对标。同样先模拟信息完整度→问"要补哪些缺口？" |

**信息完整度模拟规则**（③⑤⑥档通用）：
拿到需求后，不写完整提示词，只做快速扫描。分三类：

| 标记 | 含义 | 示例 |
|------|------|------|
| ✅ 已明确 | 用户说了，不用问 | 磨砂半透明、开机自启、显示农历 |
| ⚠️ 会拍脑袋 | 用户没说，但Agent能猜——猜错了就翻车，必须问 | 缩放方式（拖拽/预设？）、天气数据源、时辰突出方式（高亮/颜色？） |
| ❌ 缺失 | 用户没说，Agent也猜不了，缺失会跑不起来 | 节假日数据来源、字体授权 |

输出「信息完整度报告」：

```
✅ 已明确：日期格式、农历、宜忌、时辰吉凶、磨砂半透明、开机自启
⚠️ 会拍脑袋：①缩放方式（拖拽/预设？Agent会瞎猜）②天气数据源（用哪个API？）③时辰突出方式（高亮/颜色/边框？）
❌ 缺失：数字时钟位置、字体偏好、节假日数据来源
```

用户看到后，指哪补哪。说"不用"则继续当前流程——但⚠️项默认追问，❌项必须补。

**核心**：不替用户做决定。Agent能猜的≠该猜的。100-200字，三秒扫完。
...
- [ ] 输出形式判断：是否需要非文字产物（图示/文档/语音/链接）→ 见下方「输出形式扩展」
- [ ] 版本号 + 日期 + 变更摘要
...
**核心原则：给用户直观感受。** 文字不是唯一交付方式。交付前判断：纯文字够不够直观？不够就补。

| 场景 | 文字以外该补什么 | 怎么做 |
|------|-----------------|--------|
| 提示词涉及决策树/分支逻辑 | 流程图 | 生成 Draw.io 图或 Mermaid 图，嵌入文档或发链接 |
| 提示词涉及系统架构/组件关系 | 架构图 | 生成 C4 架构图或框图，嵌入文档 |
| 提示词涉及数据流/处理链路 | 数据流图 | 生成流程图或时序图 |
| 提示词涉及概念关系/知识结构 | 思维导图 | 生成思维导图 |
| 提示词涉及数据对比/统计 | 图表 | 生成柱状图、饼图、折线图等，嵌入文档 |
| 提示词需要演示效果 | 播客/视频 | 生成播客音频或视频演示 |
| 提示词涉及外部参考资源 | 链接 | 直接发链接，让用户点击即看 |
| 需要正式交付给客户/团队 | 文档 | 生成 .docx / .pptx / PDF 带图片的正式文档 |

**判断流程**：
1. 闸门六收尾时，扫一遍提示词内容
2. 问自己：纯文字用户能直观理解吗？
3. 不能 → 判断需要哪种补充形式 → 生成 → 随文字一起交付
4. 能 → 跳过

**专业用户降级**：审查模式、或用户输入结构清晰逻辑自洽 → 默认只产出文字提示词。不主动补图、文档、语音。除非用户明确说"帮我画个图"、"做个文档"。

**格式规则**：
- 图示类：先生成图片，再嵌入文档或直接发链接
- 文档类：用 docx 技能生成带图片的 .docx，或用 PPT 技能生成演示文稿
- 语音类：用播客生成音频
- 链接类：直接发 URL，不包装
