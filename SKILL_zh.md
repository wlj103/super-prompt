...
| 档位 | 用户输入特征 | 细分 | 走什么流程 |
|------|------------|------|-----------|
| **第一档：全走** | 一句话/无结构短文 | ①纯一句"帮我写个提示词"| 四层漏斗全走：收问题→拆假设→挖本体→找差集 |
| | | ②有方向但缺关键参数（如"做个记账App"没说平台）| 收问题+拆假设精简，重点挖本体补关键信息 |
| **第二档：半走** | 需求清晰完整、逻辑自洽，但非结构化文档 | ①需求描述型（如黄历280字）| 确认理解→**必须问"要不要深化需求？"**→要则联网搜再追问，不要则继续闸门零 |
| | | ②诊断型（"帮我看看这个提示词有没有问题"）| 直接反歧义18条扫描，不走过场不走四层漏斗 |
| **第三档：不走** | 完整文档/专业用户 | ①拿了已有提示词来优化 | 审查模式：全量预搜→批量抛优化点→用户选→闸门三四五六 |
| | | ②拿了完整PRD/方案 | 审查模式全量预搜对标→闸门三四五六 |

**第二档深化规则**：用户说"要深化"后，先联网搜索用户需求领域，根据搜索结果判断：有没有用户没提到的常见需求？有没有同类产品的典型功能？→ 有则按四层漏斗逻辑追问，没有则告知"搜索后没有发现需要补充的"→继续闸门零。
