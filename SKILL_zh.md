...
| **③需求清晰** | 描述完整、逻辑自洽，不追问也能理解（如黄历280字） | 确认理解后，**必须问"要不要深化需求？"**→要则联网搜再追问→继续闸门零；不要则直接闸门零 |
| **④诊断型** | "帮我看看这个提示词有没有问题" | 直接反歧义18条扫描，不走过场不走四层漏斗 |
| **⑤已有提示词优化** | 拿了现成提示词来优化 | 审查模式：全量预搜→批量抛优化点→用户选。**也问"要不要深化需求？"**→要则联网搜再追问 |
| **⑥完整文档** | 完整PRD/方案/结构化文档 | 审查模式全量预搜对标。**也问"要不要深化需求？"**→要则联网搜再追问 |

**深化规则**：③⑤⑥档位用户说"要深化"后，先联网搜索用户需求领域，根据搜索结果判断：有没有用户没提到的常见需求？有没有同类产品的典型功能？→ 有则按四层漏斗逻辑追问，没有则告知"搜索后没有发现需要补充的"→继续当前流程。①②档不需要问（本身就在挖需求），④档不需要问（用户是来诊断的）。
